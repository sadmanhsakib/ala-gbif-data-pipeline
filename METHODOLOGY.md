...
because they modify the _character_ of risk (when, how visibly, how severely)
...
1. The model learned _why_ risk is high at certain segments (ecological and
   infrastructure feature relationships)
2. The model learned _where_ risk is high (geographic proximity to high-risk
   training segments)
...
_This document should be read alongside the README for system context and the
inline docstrings in `scripts/model.py` for implementation detail._
